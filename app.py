...
#creating databse constants
sql_pass = "root1234"
app.config['SECRET_KEY'] = 'any-secret-key-you-choose'
app.config['SQLALCHEMY_DATABASE_URI'] = f"mysql://root:{sql_pass}@localhost/OS" #mysql uri
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False
db = SQLAlchemy(app) #creating sqlalchemy object
# creating table named user_info
class User_info(db.Model) : 
    # defining columns
    id = db.Column(db.Integer , primary_key = True) # defining primary key
    name = db.Column(db.String(50),nullable = False,unique = False) 
    email = db.Column(db.String(50),nullable = False , unique = False)
    message = db.Column(db.String(200),nullable = False , unique = False)
...
def home():
    if request.method == "POST" : #handling post request
        name = request.form['name'] #getting user name 
        email = request.form['email'] #getting user email 
        msg = request.form['msg']  #getting the message entered by user 
        new_info = User_info(name,email,msg) #pushing values into user info
        db.session.add(new_info) #adding new row in database
        db.session.commit() #committing changes
        connection = smtplib.SMTP("smtp.gmail.com") #establishing connection with gmail smtp server
        connection.starttls() #starting connection
        connection.login(user="jinayshah2205@gmail.com",password="spvfnyaokpczxdkf")  #logging into gmail
        # creating mail message
        connection.sendmail(from_addr="jinayshah2205@gmail.com",to_addrs=email,msg=f"Subject:Jinay\n\nDekh le ho raha hai")
